...
const {LAST_ONLINE_USER_KEY} = require('./lib/constant')
...
const {
  ...
  NAME_LENGTH,
  TABLE_NAME_KEY,
  // ONLINE_TABLE_NAME_KEY,
} = constant
const {encode, decode, arrayBufferToBase64, base64ToArrayBuffer, hashString} =
  utils
...
class CryptoStorage {
  ...
  async create() {
    ...
    if (
      ...
      throw new Error(
        `password should be a string of ${PASSWORD_LENGTH} characters`,
      )
    }

    this._setTableName(name)
    this._storage.setItem(LAST_ONLINE_USER_KEY, JSON.stringify(this._ctx))
  ...
  async use() {
    if (!this._ctx) {
      await this._openPreviousSession()
      return
    }
  ...
  async setItem(key, value) {
    ...
    if (!key || !value)
      throw new Error(
        'key (String) and value (String | Array) args are required',
      )
  ...
  close() {
    this._userPassword = null
    this._userName = null
    this._storage.removeItem(LAST_ONLINE_USER_KEY)
  }
  ...
  async _openPreviousSession() {
    const rawCtx = this._storage.getItem(LAST_ONLINE_USER_KEY)
    if (!rawCtx) {
      throw new Error('This not previous session stored')
    }

    const ctx = JSON.parse(rawCtx)
    this._ctx = ctx
    this._userName = await getEncryptedEntity(ctx.name)
    this._userPassword = await getEncryptedEntity(ctx.password)
    return true
  }
  ...
  _checkTableName() {
    ...
    if (!rawTable || !tableName.includes(hashName)) {
      throw new Error(
        'This name is not registered, you should create an instance instead',
      )
    }
    return true
  }
